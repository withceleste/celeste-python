...
from celeste import Provider, get_model
...
@pytest.mark.parametrize(
    ("mappers", "name", "value", "path", "expected", "model", "initial_request"),
    [(*case, MODEL, None) for case in SCALAR_PARAMETER_CASES]
    + [
        (
            BFL,
            IP.ASPECT_RATIO,
            "520x512",
            (),
            {"width": 520, "height": 512},
            get_model(model_id, provider=Provider.BFL),
            None,
        )
        for model_id in (
            "flux-2-max",
    ...
    + [
        ...
        for model_id in ("flux-kontext-pro", "flux-kontext-max", "flux-pro-1.1-ultra")
    ]
    + [
        (
            BFL,
            IP.PROMPT_UPSAMPLING,
            value,
            (),
            {"disable_pup": not value},
            get_model(model_id, provider=Provider.BFL),
            None,
        )
        for model_id in ("flux-2-max", "flux-2-pro", "flux-2-pro-preview")
    ...
    + [
        (
            ...
            (),
            {"safety_tolerance": 6},
            get_model("flux-2-pro", provider=Provider.BFL),
            None,
        ),
        (
            BFL,
            ...
            get_model("flux-dev", provider=Provider.BFL),
            None,
        ),
    ]
    + [
        (
            BFL,
            IP.REFERENCE_IMAGES,
            [IMAGE] * (total - int(editing)),
            (),
            {
                "input_image": "primary" if editing else IMAGE.url,
                **{f"input_image_{n}": IMAGE.url for n in range(2, total + 1)},
            }
            if total == limit
            else ConstraintViolationError,
            get_model(model_id, provider=Provider.BFL),
            {"input_image": "primary"} if editing else None,
        )
        for model_id, limit in (
            ("flux-2-pro", 8),
            ("flux-2-klein-4b", 5),
            ("flux-kontext-pro", 4),
        )
...
def test_scalar_parameters_use_provider_wire_shape(
    ...
    expected: object,
    model: Model,
    initial_request: dict[str, Any] | None,
) -> None:
    if isinstance(expected, type) and issubclass(expected, Exception):
        with pytest.raises(expected):
            _map(mappers, name, value, initial_request, model)
        return
    request = _map(mappers, name, value, initial_request, model)
