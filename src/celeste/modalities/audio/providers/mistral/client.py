"""Mistral audio client."""

from typing import Any

from celeste.artifacts import AudioArtifact
from celeste.parameters import ParameterMapper
from celeste.providers.mistral.audio import config
from celeste.providers.mistral.audio.client import (
    MistralAudioClient as MistralAudioMixin,
)
...
class MistralAudioClient(MistralAudioMixin, AudioClient):
    """Mistral audio client (speech-to-text)."""

    _transcribe_endpoint = config.MistralAudioEndpoint.CREATE_TRANSCRIPTION
    ...
    def _init_request(self, inputs: AudioInput) -> dict[str, Any]:
        ...
        if isinstance(audio, list):
            if len(audio) != 1:
                msg = "Mistral transcription accepts exactly one audio file"
                raise ValueError(msg)
            artifact = audio[0]
        else:
            artifact = audio
        if not isinstance(artifact, AudioArtifact):
            msg = "Mistral transcription requires an AudioArtifact"
            raise ValueError(msg)
    ...
    def _parse_finish_reason(self, response_data: dict[str, Any]) -> AudioFinishReason:
        """Mistral transcription does not provide finish reasons."""
        _ = response_data
