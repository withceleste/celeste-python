...
class GroqChatClient(APIMixin):
    """Mixin for Groq Chat API capabilities.

    Provides shared implementation for all capabilities using the Chat API:
    - _make_request() - HTTP POST to /openai/v1/chat/completions
    - _make_stream_request() - HTTP streaming to /openai/v1/chat/completions
    ...
    Usage:
        class GroqTextGenerationClient(GroqChatClient, TextGenerationClient):
    ...
    async def _make_request(
        ...
        **parameters: Any,
        ...
        """Make HTTP request to Groq Chat API endpoint."""
        if endpoint is None:
            endpoint = config.GroqChatEndpoint.CREATE_CHAT_COMPLETION
    ...
    def _make_stream_request(
        ...
        **parameters: Any,
        ...
        """Make streaming request to Groq Chat API endpoint."""
        if endpoint is None:
            endpoint = config.GroqChatEndpoint.CREATE_CHAT_COMPLETION
    ...
    def map_usage_fields(usage_data: dict[str, Any]) -> dict[str, int | float | None]:
        """Map Groq usage fields to unified names.
    ...
    def _parse_usage(
        ...
        return GroqChatClient.map_usage_fields(usage_data)

    def _parse_content(self, response_data: dict[str, Any]) -> Any:
        """Parse choices array from Chat API response.

        Returns raw choices array that capability clients extract from.
        """
