...
class MoonshotChatClient(APIMixin):
    ...
    - _make_stream_request() - SSE streaming with usage inclusion
    - _parse_usage() - Extract usage from response
    - _parse_content() - Extract choices from response
    - _parse_finish_reason() - Extract finish_reason from choices[0]
    - _build_metadata() - Filter content fields

    Usage:
        class MoonshotTextGenerationClient(MoonshotChatClient, TextGenerationClient):
    ...
    async def _make_request(
        ...
        **parameters: Any,
        ...
        """Make HTTP request to Moonshot Chat endpoint."""
        if endpoint is None:
            endpoint = config.MoonshotChatEndpoint.CREATE_CHAT
    ...
    def _make_stream_request(
        ...
        **parameters: Any,
        ...
        """Make HTTP streaming request returning SSE events."""
        if endpoint is None:
            endpoint = config.MoonshotChatEndpoint.CREATE_CHAT
    ...
    def map_usage_fields(usage_data: dict[str, Any]) -> dict[str, int | float | None]:
        """Map Moonshot usage fields to unified names.
